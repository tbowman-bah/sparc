...
echo -e "${BLUE}🚀 SPARC CLI Git Test${NC}"
echo "============"

# Generate unique app name with timestamp
app_name="sparc-git-$(date +%s)"
...
# Initialize git and run test
echo -e "\n${GREEN}Running test command...${NC}"
flyctl ssh console --app "$app_name" --command "/bin/bash -c 'cd /home/sparc_user && git init && git config --global user.email \"test@example.com\" && git config --global user.name \"Test User\" && sparc --cowboy-mode -m \"Create a simple Python script that prints \\\"Hello from Git!\\\" and commit it to the repository\"'"
